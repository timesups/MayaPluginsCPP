...
void AbcWriteJob::setup(double iFrame, MayaTransformWriterPtr iParent, GetMembersMap& gmMap)
{
    ...
    else if (fnDepNode.typeName() == "xgmSplineDescription")
    {
        ...
        if (status != MS::kSuccess)
        {
            MString msg = "Initialize xgmSplineDescription node ";
            msg += mCurDag.fullPathName();
            msg += " failed, skipping.";
            MGlobal::displayWarning(msg);
            return;
        ...
        if (iParent != NULL)
        {
            ...
            if (spline->isAnimated() && mShapeTimeIndex != 0)
            {
                ...
                mStats.mCurveAnimCVs += spline->getNumCVs();
            }
            else
            {
                mStats.mCurveStaticNum++;
        ...
        {
            MString err = "Can't translate ";
            err += fnDepNode.name() + " since it doesn't have a parent.";
            MGlobal::displayError(err);
        }
...
bool AbcWriteJob::eval(double iFrame)
{
    ...
    else
    {
        ...
        if (checkFrame != mShapeFrames.end())
        {
            ...
            for (spIt = mSplineList.begin(); spIt != spEnd; spIt++)
            {
                (*spIt)->write();
